...
- Lock screen motion, deliberately left for later (CD-47, D-0067): the Energy Core on the lock and setup screens is driven by page-side CSS and the Web Animations API, NOT by the wgpu compositor. Coupling it to the shell shader (so the Pulse Grid itself reacts to the charge, and the unlock flare is one continuous movement from the core into the workspace rather than a page animation followed by a view swap) needs the background to accept a transient per-frame drive signal, which is the same plumbing the Pulse Grid activity coupling above wants. The two should be built together. Also deferred: the 2FA Windows Hello step has no motion of its own (the host holds a modal OS dialog there, so the page can only report it), and the Energy Core does not yet appear on the settings vault section, where the same charge idiom would fit the change-password flow.
